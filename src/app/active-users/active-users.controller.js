(function() {
  ...
  var today = new Date(),
      paginationSize = 20,
      dailyActiveUserCompareIndex = 7,
      defaultCompareIndex = 1;
  ...
  var lastMonth = new Date(today.getTime()),
      defaultStartDate = new Date('2015-11-07'),
      defaultEndDate = new Date('2015-11-30');
  ...
  function ActiveUsersController(ActiveUsers, LinechartUtils, CSVparserUtils, APP_CONFIG, $filter) {
    ...
    vm.availableRoles = ['child', 'parent'];
    vm.availableDevices = ['android', 'ios'];
    vm.availablePieChartFilters = ['roles', 'devices'];
    vm.selectedRoleFilter = 'all';
    vm.selectedDeviceFilter = 'all';
    vm.selectedActivenessCnt = 1;
    vm.selectedView = 'Daily Active Users';
    vm.availableViews = ['Daily Active Users', 'Weekly Active Users', 'Monthly Active Users'];
    ...
    function lineChartFilter() {
      ...
      if (vm.selectedActivenessCnt && vm.selectedActivenessCnt > 0 && vm.selectedActivenessCnt <= 100) {
        ActiveUsers.getDailyActiveUsersData(vm.dailyActiveUsersDataForLineChart,
    ...
    function downloadCSV(selectedSubject) {
      var fields = ['date', 'count'],
          filename = ActiveUsers.getCsvFileName(selectedSubject, vm.dateRange.startDate, vm.dateRange.endDate),
          copyContainer;
  ...
  ActiveUsersController.$inject = ['ActiveUsers', 'LinechartUtils', 'CSVparserUtils', 'APP_CONFIG', '$filter'];

  angular.module('dataDashboard.activeUsers.controller.ActiveUsersController', [])
