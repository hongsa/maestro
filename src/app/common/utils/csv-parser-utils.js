(function() {
      ...
      for (var i=1; i < parsedCSV.length; i++) {
        if (parsedCSV[i].length > 3) {
          for (var j=0; j < columns.length; j++) {
            dataContainer[j].data.push([new Date(parsedCSV[i][0]).getTime(), parseInt(parsedCSV[i][columns[j]])]);
          }
        }
      }
    }
  ...
  angular.module('dataDashboard.common.utils.CSVparserUtils', [])
    .factory('CSVparserUtils', CSVparserUtils);
})();