  ...
  function CumulativeUserData($http, $q, $filter, APP_CONFIG) {
    ...
    function getCumulativeUserData(dataContainer, selectedRange, paidFilter, startDate, endDate) {
      var deferred = $q.defer(),
          query = createQueryString(paidFilter),
          prevValue = 0,
          criteria = -1,
          compareNum;
        ...
        $http({
          ...
          if (!result.data.timed_out && result.data.length > 0) {
            ...
            result.data.forEach(function (item) {
              ...
              } else if (getTimeStampFromStr(startDate) < getTimeStampFromStr(item.created_at)
                  && getTimeStampFromStr(endDate) >= getTimeStampFromStr(item.created_at)) {
