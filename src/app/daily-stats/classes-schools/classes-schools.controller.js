(function () {
  ...
  function ClassesSchoolsStatsController(DailyStats, LinechartUtils, CSVparserUtils, APP_CONFIG) {
    ...
    function updateData() {
      ...
      DailyStats.getDailyStatsData(vm.newClasses, 'newClasses', DailyStats.dateRange.startDate, DailyStats.dateRange.endDate, vm.selectedDeviceFilter, vm.selectedRoleFilter, vm.selectedCountryFilter, vm.linechart);
      DailyStats.getDailyStatsData(vm.deletedClasses, 'deletedClasses', DailyStats.dateRange.startDate, DailyStats.dateRange.endDate, vm.selectedDeviceFilter, vm.selectedRoleFilter, vm.selectedCountryFilter, vm.linechart);
    ...
    function fetchAndDownloadCSV(selectedSubject) {
      if (selectedSubject === 'New Classes') {
        CSVparserUtils.downloadCSV(vm.newClasses, false, 'daily_new_classes_' + DailyStats.dateRange.startDate.toLocaleDateString().replace(/[\.\ ]/g, '') + '-' + DailyStats.dateRange.endDate.toLocaleDateString().replace(/[\.\ ]/g, '') + '_by_device-' + vm.currentFilter.device + '_country-' + vm.currentFilter.country + '.csv');
      } else if (selectedSubject === 'Deleted Classes') {
        CSVparserUtils.downloadCSV(vm.deletedClasses, false, 'daily_deleted_classes_' + DailyStats.dateRange.startDate.toLocaleDateString().replace(/[\.\ ]/g, '') + '-' + DailyStats.dateRange.endDate.toLocaleDateString().replace(/[\.\ ]/g, '') + '_by_device-' + vm.currentFilter.device + '_country-' + vm.currentFilter.country + '.csv');
      }
    }
  ...
  ClassesSchoolsStatsController.$inject = [
    'DailyStats',
    'LinechartUtils',
    'CSVparserUtils',
    'APP_CONFIG'
  ];
  angular.module('dataDashboard.dailyStats.controller.ClassesSchoolsStatsController', []).controller('ClassesSchoolsStatsController', ClassesSchoolsStatsController);
}());