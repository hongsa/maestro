(function () {
  ...
  function PostStatsController(DailyStats, LinechartUtils, CSVparserUtils, APP_CONFIG) {
    ...
    function updateData() {
      ...
      DailyStats.getDailyStatsData(vm.newPosts, 'newPosts', DailyStats.dateRange.startDate, DailyStats.dateRange.endDate, vm.selectedDeviceFilter, vm.selectedRoleFilter, vm.selectedCountryFilter, vm.linechart);
    ...
    function fetchAndDownloadCSV() {
      CSVparserUtils.downloadCSV(vm.newPosts, false, 'daily_new_posts_' + DailyStats.dateRange.startDate.toLocaleDateString().replace(/[\.\ ]/g, '') + '-' + DailyStats.dateRange.endDate.toLocaleDateString().replace(/[\.\ ]/g, '') + '_by_device-' + vm.currentFilter.device + '_role-' + vm.currentFilter.role + '_country-' + vm.currentFilter.country + '.csv');
    }
  ...
  PostStatsController.$inject = [
    'DailyStats',
    'LinechartUtils',
    'CSVparserUtils',
    'APP_CONFIG'
  ];
  angular.module('dataDashboard.dailyStats.controller.PostStatsController', []).controller('PostStatsController', PostStatsController);
}());