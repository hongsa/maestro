(function() {
  ...
  function UserStatsController(DailyStats, LinechartUtils, CSVparserUtils, APP_CONFIG) {
    ...
    function updateData() {
      ...
      DailyStats.getDailyStatsData(vm.newUsers,
                                   'newUsers',
                                   ...
                                   vm.linechart);
      DailyStats.getDailyStatsData(vm.deletedUsers,
                                   'deletedUsers',
                                   DailyStats.dateRange.startDate,
                                   DailyStats.dateRange.endDate,
                                   vm.selectedDeviceFilter,
    ...
    function fetchAndDownloadCSV(selectedSubject) {
      if (selectedSubject === 'New Users') {
        CSVparserUtils.downloadCSV(vm.newUsers,
                                   false,
                                   'daily_new_users_' +
        ...
        CSVparserUtils.downloadCSV(vm.deletedUsers,
                                   false,
                                   'daily_deleted_users_' +
                                   DailyStats.dateRange.startDate.toLocaleDateString().replace(/[\.\ ]/g, '') + '-' +
                                   DailyStats.dateRange.endDate.toLocaleDateString().replace(/[\.\ ]/g, '') + 
                                   '_by_device-' + vm.currentFilter.device +
                                   '_role-' + vm.currentFilter.role +
  ...
  UserStatsController.$inject = ['DailyStats', 'LinechartUtils', 'CSVparserUtils', 'APP_CONFIG'];

  angular.module('dataDashboard.dailyStats.controller.UserStatsController', [])
    .controller('UserStatsController', UserStatsController);
})();