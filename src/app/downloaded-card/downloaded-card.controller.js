(function() {
  ...
  function DownloadedCardController(DownloadedCard, LinechartUtils, PiechartUtils, APP_CONFIG, $filter, CSVparserUtils) {
    ...
    vm.availableRanges = ['daily', 'weekly', 'monthly'];
    vm.availablePaid = ['free', 'paid'];
    vm.availablePubishers = [];

    vm.selectedRange = 'daily';
    vm.selectedPaidFilter = 'all';
    vm.selectedPublisherFilter = '5';
    ...
    function lineChartFilter() {
      DownloadedCard.getCumulativeUserData(vm.downloadedCardForLineChart,
                                           vm.selectedRange,
                                           vm.selectedPublisherFilter,
    ...
    function getPublisher() {
      DownloadedCard.getPublisherKeys(vm.availablePubishers,
          vm.dateRange.startDate,
    ...
    function exportPDF() {
      var doc = new jsPDF('p', 'pt'),
          columns = ["Date", "Downloaded Cards", "Change Net / %"],
          tmp = [];
      ...
      for (var i = 0; i < vm.reversedData.length; i++) {
        if (i === vm.reversedData.length-1) {
          ...
          tmp.push([
            dateFormat(new Date(vm.reversedData[i][0])),
            vm.reversedData[i][1],
            (vm.reversedData[i][1] - vm.reversedData[i+1][1]).toString() + ' / ' +
            (Math.round(((vm.reversedData[i][1] - vm.reversedData[i+1][1]) / vm.reversedData[i+1][1]) * 100) / 100).toString() + ' %'
          ])
        }
      }

      doc.autoTable(columns, tmp,{
        theme: 'grid'
      });
      doc.save('table.pdf');
    }
    ...
    function fetchAndDownloadCSV(dataContainer) {
      var copyContainer = [],
          fields;
      fields = ['Date', 'downloaded Cards', 'Change Net/%'];
  ...
  DownloadedCardController.$inject = ['DownloadedCard', 'LinechartUtils', 'PiechartUtils', 'APP_CONFIG', '$filter','CSVparserUtils'];

  angular.module('dataDashboard.downloadedCard.controller.DownloadedCardController', [])
    .controller('DownloadedCardController', DownloadedCardController);
})();
