(function() {
  ...
  function UserContacts($http, $q, APP_CONFIG, CSVparserUtils) {
    return {
      fetchAndDownloadActiveTeachersContacts: fetchAndDownloadActiveTeachersContacts
    };

    function fetchAndDownloadActiveTeachersContacts(subject, yearFilter, monthFilter, countryFilter) {
      var deferred = $q.defer(),
          filename = getFilename(subject, yearFilter, monthFilter, countryFilter);
    ...
    function getFilename(subject, yearFilter, monthFilter, countryFilter) {
      if (subject === 'user-contacts') {
        var dateCopy = new Date([yearFilter, parseInt(monthFilter)+1, '01'].join('-')),
            startDate = getDateInString(dateCopy),
            endDate;
  ...
  UserContacts.$inject = ['$http', '$q', 'APP_CONFIG', 'CSVparserUtils'];

  angular.module('dataDashboard.userContacts.service.UserContacts', [])
    .factory('UserContacts', UserContacts);
})();
