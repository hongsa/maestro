(function() {
  ...
  function UserPageFlow($http, $q, APP_CONFIG) {
    ...
    function getUserPageFlow(dataContainer, userId, startDate, endDate, alertCb) {
      var deferred = $q.defer(),
          query = createQueryString(userId, startDate, endDate);
      ...
      $http({
        ...
        if (result.data.timed_out || result.data.hits.hits.length === 0) {
          ...
          result.data.hits.hits.forEach(function(row) {
            if ((!dataContainer[dataContainer.length-1] ||
                 (dataContainer[dataContainer.length-1].cur_page !== row._source.cur_page &&
                  dataContainer[dataContainer.length-1].prev_page !== row._source.prev_page)) &&
                (row._source.cur_page !== '' && row._source.prev_page !== '')) {
    ...
    function createFromRangeString(startDate, endDate) {
      var startDateCopy = new Date(startDate.getTime()),
          ...
          from = ' FROM ' + formatDateName(devicePrefix, getDateInNumbers(startDate));
  ...
  UserPageFlow.$inject = ['$http', '$q', 'APP_CONFIG'];

  angular.module('dataDashboard.userPageFlow.service.UserPageFlow', [])
    .factory('UserPageFlow', UserPageFlow);
})();
